...
extract_rootfs_archive() {
	if [ -z "${KEEP_ROOTFS_DIRECTORY}" ]; then
		...
		if # unzip -p "${ARCHIVE_NAME}" "kalifs-${SYS_ARCH}-${SELECTED_INSTALLATION}.tar.xz" |
			proot --link2symlink tar --strip=1 --delay-directory-restore --warning=no-unknown-keyword --extract --xz --exclude="dev" --file="-" --directory="${ROOTFS_DIRECTORY}" --checkpoint=1 --checkpoint-action=ttyout="${I}${Y}    Extracted %{}T in %ds%*\r${N}${V}" &>>"${LOG_FILE}"
		then
